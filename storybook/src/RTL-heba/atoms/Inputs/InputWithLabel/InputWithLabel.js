...
export const InputWithLabel = ({ label, type, placeholder }) => {
  ...
  return (
    <div title="InputWithLabel">
      <label className="block my-2 text-xs  text-dark-grey">{label}</label>
      <div
        className={`
          flex items-center  w-full px-m py-xs relative  rounded-md  text-sm text-grey-shd1 hover:border-grey-shd2 focus:border-dark
          ${style}`}
      >
        <input
          type="text"
