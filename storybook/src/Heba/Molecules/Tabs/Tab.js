...
export const Tab = ({ content, icon, status, styles }) => {
  const style =
    styles === TAB.STYLES.ACTIVE
      ...
      : styles === TAB.STYLES.HOVER
      ? "bg-grey-shd6 px-4 h-8 py-1.5  rounded-full"
      : "bg-grey-shd6 px-4 h-8 py-1.5 rounded-full opacity-60";

  return (
    <>
      <button className={`flex items-center ${style} ml-2 focus:outline-none`}>
        <Radio size="Small" status={status} />
        {icon === TAB.ICON.IMAGE && (
          <svg
            className="ml-2"
            width="16"
