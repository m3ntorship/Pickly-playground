...
function InputImage({ fullRound, width = "400px", icon, placeholder }) {
  ...
  switch (fullRound) {
    ...
    case false: {
      round = "rounded-bl-md rounded-br-md";
      break;
    }
    default: {
      round = "rounded-md ";
  ...
  return (
    <div
      className={`${round} h-10  border-grey-shd5 border mt-1 mb-12 pr-4 flex align-middle items-center`}
      style={{ width }}
    >
      {/* Icon Letter A */}
      <svg
      ...
      <input
        ...
        //width="644px"
        style={{ width }}
        className="h-5 text-sm font-sans outline-none ml-2 text-grey-shd1"
      />
    </div>
  );
}
